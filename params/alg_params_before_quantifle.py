...
def _hparams(algorithm, dataset, random_seed, args):
    ...
    # -----------------------------------------------------------------------
    # --------------------- Lifeis
    elif algorithm in ["Lifeis"]:
        _hparam('swapping_loss_weight', 1, lambda r: 10 ** r.uniform(-2, 0))

    elif algorithm in ["Lifeis_up"]:
        _hparam('is_descending', True, lambda r: bool(r.choice([True, False])))
        _hparam('keep_rate', 1,  lambda r: r.choice([0.6, 0.7, 0.8, 0.9, 1]))
        _hparam('swapping_loss_weight', 0.1, lambda r: 10 ** r.uniform(-2, 0))
    ...
    # ------------------------------------------------------------------------
        ...
        _hparam('vne_coef', 0.001,  lambda r: r.choice([-0.05, -0.01, -0.005, -0.001,
