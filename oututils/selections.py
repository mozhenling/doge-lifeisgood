...
class IIDAccuracySelectionMethod(SelectionMethod):
    ...
    @classmethod
    def run_acc(self, run_records, is_end_acc =True):
        test_records = get_test_records(run_records)
        if not len(test_records):
            return None
        if is_end_acc:
            # report the accuracies over the last checkpoint of a single model training
            # this requires multiple trials of the total numer of steps
            chosen_record = run_records.sorted(lambda r: r['step'])[-1]
            return self._step_acc(chosen_record)
        else:
            # report the accuracies over all checkpoints of a single model training
            # this may lead to over-fitting of the training data
